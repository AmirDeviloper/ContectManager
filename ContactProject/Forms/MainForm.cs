        ...
        private void remove_contact_button_Click(object sender, EventArgs e)
        {
            if(contact_grid.CurrentRow != null)
            {
                using (var db = new UnitOfWork())
                {
                    ...
                    var transactions = db.ContactTransactionRepository.Get(c => c.ContactID == selected_contact_id).ToList();

                    var info = "آیا از حذف مخاطب انتخابی , اطمینان دارید؟ این مخاطب دارای لیستی از شماره تماس ها و ایمیل میباشد. همچنین تراکنش هایی ممکن است داشته باشد\n\n";
                    ...
                    info += "\nمجموع پرداختی: ";
                    info += $"{transactions.Where(t => t.IsPayment).Select(t => (int)t.Value).Sum()}\n";
                    ...
                    if (MessageBox.Show(info, "تأیید حذف مخاطب", MessageBoxButtons.YesNo, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1, MessageBoxOptions.RightAlign) == DialogResult.Yes)
                    {
                        ...
                        {
                            db.ContactTransactionRepository.Delete(transaction);
                        }
